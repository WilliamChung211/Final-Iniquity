...
;; calls the function f with the parameters of the expressions in the list e
;; which should be a pointer to a pair or empty
;; Variable Expr CEnv -> Asm
(define (compile-apply f e c)
 (let ((c1 (compile-e e c))
   ...
   `(
     ...
     ;; compiles the list which should be a list
     ,@c1
      (mov rbx 0)

      ;; puts the counter of arguments in the list in the place
      ;; that is 8 bits down where the rsp will be after a call
      ;; to f since this argument number will be the first argument
      ;; in the parameters
      (mov (offset rsp ,(- (add1 (length (cons #f c))))) rbx)

      

      ;; this register contains a pointer to the next spot on the stack to
      ;; put an argument
      (mov rcx rsp)
      (sub rcx ,(* (add1 (add1(length (cons #f c)))) 8))
      ...
      (cmp rbx ,imm-type-empty)
      (je ,l0)
      
      ;; assert the register is containing a pointer to a pair
      ,@assert-pair
      (xor rax ,type-pair)

      ;; copies the car of the list to the stack
      (mov rbx (offset rax 0))
      (mov (offset rcx 0) rbx)

      ;; advances the register's pointer address 8 spots down the stack
      (sub rcx 8)

      ;; retrieves, increments, and stores the counter of e's arguments
      ;; since the ocunter should be as big as the number of elements in
      ;; the list
      (mov rbx (offset rsp ,(- (add1 (length (cons #f c))))))
      (add rbx 1)
      (mov (offset rsp ,(- (add1 (length (cons #f c))))) rbx)

      ;; sets the register holding the list to the list's cdr
      ...
      ;; moves the rsp down to the end of the arguments
      ;; so that it can decrement rsp by 8, put the PC in the
      ;; rsp and call f
      (sub rsp ,stack-size)
      (call ,(symbol->label f))
      (add rsp ,stack-size))
...
(define (compile-call f es c)
  ...
  (let ((cs (compile-es es (cons #f (cons #f c))))
        (stack-size (* 8 (length c))))
    `(,@cs

      ;; communicates arity info by putting the number of given
      ;; arguments in the place  that is 8 bits down where the rsp
      ;; will be after a call to f since this argument number will
      ;; be the first argument in the parameters
      (mov rax ,(length es))
      (mov (offset rsp ,(- (add1 (length (cons #f c))))) rax)
      (sub rsp ,stack-size)
      (call ,(symbol->label f))
      (add rsp ,stack-size))))
    ...
    [(fundef name args body)
      (match args
        [(list xs ...)
         ...
         ;; the first argument will be the size of the rest of the arguments
         (let ((c0 (compile-e body (reverse (cons #f xs)))))
           `(,(symbol->label name)

             ;; checks if the first argument is the same as the size
             ;; of the length of the expected number of arguments
             ;; (not including this first one)
             (mov rax ,(length xs))
             (mov rbx (offset rsp -1))
             (cmp rax rbx)
        ...
        [(list* xs ... r)
         ...
         ;; the stack will now include the list r, xs, and the first argument
         ;; that should be greater or equal the number of xs
         (let ((c0 (compile-e body (reverse (cons #f (append xs (list r))))))(l0 (gensym))(l1 (gensym)))
           `(
             ,(symbol->label name)
             ...
             ;; if length of xs is greater than the number of arguments, error
             (jg err)

             ;; rbx is set to arguments that are not the counter or any arguments in xs
             ;; this will help keep track of the arguments that are left on the stack to
             ;; add to the list on the stack spot after the slots that arguments in xs
             ;; take up
             (sub rbx rax)
            
             ;; tracks where to put next element/cdr (a pair containing the next element as car
             ;; or an empty list)
             (mov rcx rsp)
             (sub rcx ,(* (add1(length (cons #f xs))) 8))
             
             ;; tracks what address in stack holds the element that will be added to
             ;; the list next 
             (mov rdx rcx)
             ...
             ;; if there are no more arguments to add from the stack to the list, we can
             ;; end the loop and just add the empty list
             (mov rax 0)
             (cmp rbx rax)
             (je ,l1)

             ;; retrieves the element in the slot that we must put in as the car of the pair
             ;; in the specifc slot
             (mov rax (offset rdx 0))

             ;; puts the car on the heap and moves the pair representation to the specific spot
             ;; where we need to put this pair
             (mov (offset rdi 0) rax)
             (mov rax rdi)
             (or rax ,type-pair)
             (mov (offset rcx 0) rax)

             ;; make rcx the cdr of this pair by moving it up 8 spots in the heap
             (mov rcx rdi)
             (add rcx 8)

             ;; go to the next slot of the next element to be added
             (sub rdx 8)
             (add rdi 16)
             (sub rbx 1)
             (jmp ,l0)
             ...
             ;; end by putting either the cdr of the last pair (if size of r doesn't = 0)
             ;; or the last argument spot as (if not)
